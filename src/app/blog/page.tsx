	...
	return (
		<>
			...
			<div className='flex flex-col items-center justify-center gap-6 px-6 pt-24 max-sm:pt-24'>
				...
				{items.length > 0 && siteContent.moreLink?.enabled && siteContent.moreLink?.url && (
					<div className='text-center'>
						<motion.a
							...
							whileHover={{ scale: 1.05 }}
							whileTap={{ scale: 0.95 }}
							href={siteContent.moreLink.url}
							target='_blank'
							className='card text-secondary static inline-flex items-center gap-2 rounded-xl px-4 py-2 text-xs'>
							{siteContent.moreLink.icon === 'juejin' && <JuejinSVG className='h-4 w-4' />}
							{siteContent.moreLink.text || '更多'}
						</motion.a>
					</div>
				)}
			</div>
